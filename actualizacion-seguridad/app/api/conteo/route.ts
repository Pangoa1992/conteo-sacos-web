 ...
 *
 * Seguridad: valida el token compartido (VISION_API_TOKEN) enviado en
 * el header Authorization, para que solo el módulo de visión pueda
 * escribir conteos nuevos. Si VISION_API_TOKEN no está configurado
 * (por ejemplo en desarrollo local), se omite la validación para no
 * bloquear las pruebas.
 */
export async function POST(req: NextRequest) {
  const tokenEsperado = process.env.VISION_API_TOKEN;

  if (tokenEsperado) {
    const authHeader = req.headers.get("authorization") || "";
    const tokenRecibido = authHeader.replace(/^Bearer\s+/i, "");
